...
class Admin_SettingController extends BaseController {
	...
	*/
	...
	public function routePage($page='',$pager_page='')
	{
		...
		switch ($page) {
			...
			case 'coins-voting':
				...
				$wallet = Wallet::where('type','MGC')->first();
				if(isset($wallet->id)){
					try{
		                ...
		                foreach ($coinvotes as $key => $value) {
		                 	...
		                 	$num_payment = floor($btc_payment/100);
                ...
                else $data['not_wallet'] = "Please add MGC wallet before add the vote coin.";
			...
			default:
				...
				$data['amount_btc_per_vote']=$setting->getSetting('amount_btc_per_vote',100);
	...
	public function updateSetting(){
		...
		$amount_btc_per_vote = is_numeric(Input::get('amount_btc_per_vote'))? Input::get('amount_btc_per_vote'):100;
