    ...
    @if($wallet->enable_deposit)
    	@if(!$wallet->is_moneypaper)
			...
			function generateQRwithGoogle($url,$widthHeight ='150',$EC_level='L',$margin='0') {
				$url = urlencode($url);
				echo '<img src="https://chart.apis.google.com/chart?chs='.$widthHeight.
		    ...
		    <div class="addressSection">
		    	<div class="options box" style="display: inline-block;"><input id="address" style="font-weight: bold; font-size: 16px; min-width:400px;border-radius:5px;border:none;padding:5px;" value="{{$address_deposit}}" readonly>&nbsp; <!--<input id="copy-button" type="button" value="Copy" class="inline btn btn-info" data-clipboard-target="address"> --></div>
		    <p class="m-t">
					<?php
