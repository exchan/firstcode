					...
					<table class="table striped hovered">
						...
						<tbody>
							...
							<?php 
								...
								$change = ($pre_price!=0)? sprintf('%.2f',(($curr_price-$pre_price)/$pre_price)*100):0;
							?>
								<tr id="volume-{{$doge_market->id}}" data-hint="Vol: {{sprintf('%.8f',$total_doge)}} BTC" data-hint-position="right">
									<td><a href="{{{ URL::to('/market/') }}}/{{$doge_market->id}}"><span class="name">{{$doge_market->type}}</span></a></td>
									<td><span class="price" yesterdayPrice="{{sprintf('%.8f',$pre_price)}}" id="spanPrice-{{$doge_market->id}}">{{sprintf('%.8f',$curr_price)}}</span></td>
									<td>
										@if($change>=0)
...
<tr><td><span class="name">DOGE Volume</span></td><td><span class="change">{{$volumn_doge}} BTC</span></td></tr>
							<tr><td><span class="name">Number of Trades</span></td><td><span class="change">{{$number_doge+$number_ltc+$number_btc}}</span></td></tr>
						</tbody>		
					</table>
				</div>
			</div>
